...
def main_current_sales():
    ...
    api_url = create_request_url(
        configs["booli"]["api_caller"], configs["booli"]["api_key"], area_id="386690"
    )
            ...
            matched_listings.append(
                ListObject(
                    ...
                    published=datetime.datetime.strptime(
                        listing.get("published"), "%Y-%m-%d %H:%M:%S"
                    ).date(),
                    url=str(listing.get("url")),
                )
            )

    pd.DataFrame(matched_listings).to_excel(
        "output_files/tripolis_for_sale.xlsx", index=False, freeze_panes=(1, 0)
    )
