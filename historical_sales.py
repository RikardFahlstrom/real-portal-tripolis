...
def main_historical_sales(runs_local=False):
    ...
    structured_data, df_graph = create_dataframes_for_table_and_linegraph(transformed_data)
...
def setup_browser(configs, runs_local=False) -> webdriver:
    ...
    if runs_local:
        browser = webdriver.Chrome(executable_path=configs['selenium']['local_executable_path'], options=options)
    else:
        options.add_argument('--headless')
        browser = webdriver.Remote("http://chrome:4444/wd/hub", webdriver.DesiredCapabilities.CHROME, options=options)
...
def get_num_pages_to_query(chrome_browser, configs):
    chrome_browser.get(configs['abj_portal']['scrape_url'] + '1')  # +1 to retrieve first pageno
    num_pages = chrome_browser.find_element_by_class_name('pagination').text

    return int(num_pages.split('\n')[-2])


def download_data(browser: webdriver, configs: configparser.ConfigParser, last_page_num) -> pd.DataFrame:
    ...
    for page in range(2, last_page_num + 1):
        ...
        df = pd.read_html(
            browser.find_element_by_xpath('/html/body/div[2]/div[1]/div[2]/center/table').get_attribute('outerHTML'),
            header=0)[0]
        all_dfs.append(df)
        time.sleep(2)
...
def transform_data(raw_data: pd.DataFrame) -> pd.DataFrame:
    ...
    raw_data_grouped = raw_data.groupby('transaction_id').agg({'Area': 'max',
    ...
    raw_data_grouped[['transaction_id', 'överlåtelse']] = raw_data_grouped['transaction_id'].str.split(';', expand=True)
    raw_data_grouped[['Pris', 'Pris_leftovers']] = raw_data_grouped['Pris'].str.split(',', expand=True)
    raw_data_grouped['Lokaltyp'] = raw_data_grouped.apply(lambda row: remove_duplicates_from_string(row['Lokaltyp']), axis=1)
    raw_data_grouped['Adress'] = raw_data_grouped.apply(lambda row: remove_duplicates_from_string(row['Adress']), axis=1)
    ...
    raw_data_grouped['överlåtelse_yearmonth'] = raw_data_grouped['överlåtelse'].dt.to_period('M')
    raw_data_grouped['overlatelse_year'] = raw_data_grouped['överlåtelse'].dt.to_period('Y')
    ...
    raw_data_grouped = raw_data_grouped[(raw_data_grouped['Pris'] != 0) & (~raw_data_grouped['Lokaltyp'].isin(['Egen lokal', 'Lokal'])) & (raw_data_grouped['pris_kvm'] >= 10000)]
...
def create_dataframes_for_table_and_linegraph(transformed_data):
    ...
    df_graph = transformed_data.groupby(['overlatelse_year']).agg(
        {'Pris': 'sum',
         'Area': 'sum',
         'overlatelse_date': 'count'}).reset_index()
...
def create_graph(structured_sales_data, df_for_line_graph):
    ...
    columns = [
        TableColumn(field="overlatelse_date", title="Överlåtelsedatum", formatter=models.DateFormatter(format="%Y-%m-%d")),
    ...
    data_table = DataTable(
        columns=columns,
        source=ColumnDataSource(structured_sales_data),
        height=600,
        )
...
def save_as_csv(formatted_sales_data: pd.DataFrame):
    formatted_sales_data.to_excel('output_files/tripolis_historical_sales.xlsx', index=False, freeze_panes=(1, 0))
